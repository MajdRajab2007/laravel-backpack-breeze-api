...
const ReadRoad = async ({params}) => {
        ...
        let req = await fetch(`https://api.yg.youth-guide.com/api/self-roadmap/${params.id}`);
  ...
  return (
        <div key={res.id} className='border-2 pb-8 rounded-2xl gap-5
          ...
          <Image className=' rounded-t-2xl md:rounded-none' src={`https://api.yg.youth-guide.com/storage/${res.image}`} height={500} width={500} alt='...'/>
