...
const page = async({params}) => {
    let req = await fetch(`http://localhost:8000/api/posts/tag/${params.id[0]}/${params.id[1]}`)
        ...
        {data.map((lesson) => {
            return (
                <div key={lesson.id} className='border-2 pb-8 border-greenPrime rounded-2xl gap-5
                        ...
                        <Image className=' rounded-t-2xl md:rounded-none' src={`http://localhost:8000/storage/${lesson.image}`} height={500} width={500} alt='...'/>
